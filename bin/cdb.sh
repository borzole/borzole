...
menu_choice=""
current_cd=""
title_file="$HOME/title.cdb"
tracks_file="$HOME/tracks.cdb"
temp_file=/dev/shm/cdb.$$
trap "rm -f $temp_file" EXIT
# ------------------------------------------------------------------------------
get_return() {
	echo -e "Press [Enter] \c"
	read x
	return 0
}
# ------------------------------------------------------------------------------
...
main_menu(){
	while : ; do
		...
		case "$menu_choice" in
			...
			q) break;;
			*)
				echo -n "Sorry, not recognized: '$menu_choice'! " >&2
				get_return;;
		esac
	done
}
