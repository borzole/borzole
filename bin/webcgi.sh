...
# server root
root=$HOME/studio/python/cgi.dev.null
# log
log=~/.log/${0##*/}.log
# ------------------------------------------------------------------------------
...
get_pid(){
	ps -P `pgrep python` | grep CGIHTTPServer | awk '{print $1}'
}
is_pid(){
	ps -P `pgrep python` | grep CGIHTTPServer &>/dev/null
}
